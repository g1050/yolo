...
GL_NUMBBOX = 2
GL_NUMGRID = 7
STATIC_DATASET_PATH = r'/data/xkgao/YOLO-World/dataset/voc2012/anyforge/pascal-voc-2012/VOC2012_train_val'
...
def convert_annotation(anno_dir, image_id, labels_dir):
    ...
    # print(image_id)
    # if (image_id != '2010_001754'):
    #     return
    for obj in root.iter('object'):
        difficult = obj.find('difficult').text
        cls = obj.find('name').text
        # print(f"cls: {cls}, difficult: {difficult}")
...
def make_label_txt(anno_dir, labels_dir):
    ...
    for file in filenames:
        print("process %s"%os.path.join(anno_dir, file))
...
def img_augument(img_dir, save_img_dir, labels_dir):
    ...
    for img_name in imgs_list:
        ...
        if STATIC_DEBUG:
            img_name = img_name.split('.')[0] + "_debug.jpg"
            cv2.imwrite(os.path.join(save_img_dir, img_name), img)
        # 由于这里是覆盖写，所以最后只有一个目标
        # print(bbox)
        with open(os.path.join(labels_dir, img_name.split('.')[0] + ".txt"), 'w+') as f:
            for i in range(len(bbox) // 5):
                bbox_line = [str(x) for x in bbox[i*5:(i*5+5)]]
                # print(bbox_line)
                str_context = " ".join(bbox_line)+'\n'
                f.write(str_context)
...
def convert_bbox2labels(bbox):
    ...
    for i in range(len(bbox)//5):
        ...
        # 将整张图片的坐标系(0-1)转换为网格内的局部坐标系(0-1),为了便于网络收敛
...
def create_csv_txt(img_dir, anno_dir, save_root_dir, train_val_ratio=0.9, padding=10, debug=False):
    ...
    # 处理成txt文件 cls x,y,w,h (中心点,w,h) 归一化后的坐标
    ...
    print(f"train_id: {len(train_id)}, test_id: {len(test_id)}")
    # train_id: 15405, test_id: 1712
